...
const AOE_KNOCKBACK      : float = 600.0
...
func _ready() -> void:
	...
	set_meta("monster_type", "slime2_boss")
...
func _build_sprite_frames() -> void:
	...
	for anim: String in ["Idle", "Walk", "Run", "Hurt", "Death", "Attack"]:
		for dir: String in ["front", "back", "left", "right"]:
			...
			var path := "%s%s/Slime2_%s_%s.aseprite" % [ASSET_BASE, anim, anim, dir]
...
func _on_died() -> void:
	...
	queue_redraw()
	SceneManager.earn_slime_goop(12)
	_sprite.flip_h = false
	match facing:

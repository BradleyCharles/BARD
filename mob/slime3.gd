...
func _build_sprite_frames() -> void:
	...
	for anim: String in ["Idle", "Walk", "Run", "Hurt", "Death", "Attack"]:
		for dir: String in ["front", "back", "left", "right"]:
			...
			var path      := ASSET_BASE + anim + "/Slime3_" + anim + "_" + dir + ".aseprite"
...
func _merge_anim(sf: SpriteFrames, anim_name: String, path: String) -> void:
	...
	var loop: bool = not (anim_name.begins_with("hurt") or anim_name.begins_with("death") or anim_name.begins_with("attack"))
...
func _fire_aoe() -> void:
	...
	queue_redraw()
	_play_attack()
	if _distance_to_player() <= AOE_RADIUS and _player_ref != null and is_instance_valid(_player_ref):
		var kb_dir: Vector2 = (_player_ref.global_position - global_position).normalized()
...
func _draw() -> void:
	if _telegraph_radius > 0.0:
		...
		# local radius: world AOE_RADIUS divided by node scale
		var local_r: float = _telegraph_radius
		draw_circle(Vector2.ZERO, local_r, Color(1.0, 0.0, 0.0, 0.25 * alpha))
		draw_arc(Vector2.ZERO, local_r, 0.0, TAU, 64, Color(1.0, 0.0, 0.0, 0.9), 2.0)
...
func take_damage(amount: int, knockback_vec: Vector2) -> void:
	...
	if health <= 0:
		return
	if _boss_phase == BossPhase.TELEGRAPH:
...
func _on_died() -> void:
	...
	queue_redraw()
	SceneManager.earn_slime_goop(8)
	_sprite.flip_h = false
	match facing:

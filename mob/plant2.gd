...
const ASSET_BASE    : String = "res://assets/mobs/Plant2/"
const MOB_RADIUS    : float  = 30.0
const STRIKE_RADIUS : float = MobStats.PLANT2_STRIKE_RADIUS
const WANDER_SPEED  : float = MobStats.PLANT2_WANDER_SPEED
const AGGRO_SPEED   : float = MobStats.PLANT2_AGGRO_SPEED
...
func _ready() -> void:
	max_health      = MobStats.PLANT2_MAX_HEALTH
	personality     = Personality.WANDER
	aggro_radius    = MobStats.PLANT2_AGGRO_RADIUS
	damage          = MobStats.PLANT2_DAMAGE
	knockback_force = MobStats.PLANT2_KNOCKBACK

	super._ready()
	_apply_hitbox(MobStats.PLANT2_HITBOX_RADIUS)

	set_meta("monster_type", "plant2")
...
func _physics_process(delta: float) -> void:
	...
	if dist < aggro_radius or _is_aggroed:
		...
		var dir := _direction_to_player_with_noise(AGGRO_SPEED)
